...
class Product(BaseModel):
    ...
    def __init__(self, name=None, description=None, cost_price=None,
                 selling_price=None, stock=None, size=None,
    ...
    def build_paginated_response(cls, products, url):
        ...
        for product in products.items:
            data = cls.get_dict(
                ...
                images=[
                    "{}uploads/{}".format(
                        request.url_root, v.lstrip()
                    ) for v in product.images.split(',')
                ],
...
class Review(db.Model):
    ...
    def __repr__(self):
        return "<%r (%r, %r)>" % (
            self.__class__.__name__, self.product_id, self.user_id)

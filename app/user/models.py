...
class User(BaseModel):
    ...
    @classmethod
    def build_paginated_response(cls, users, url):
        results = []
        for user in users.items:
            data = cls.get_dict(id=user.id, first_name=user.first_name, middle_name=user.middle_name,
                                last_name=user.last_name, email=user.email, phone=user.phone,
                                created_at=user.created_at.isoformat(),
                                updated_at=user.updated_at.isoformat())
            results.append(data)
        data = cls.build_response(users, results, url)
        return data

    @classmethod
    def get_by_id_data(cls, id):
        user = cls.get_by_id(id)
        data = user.get_dict(
            id=user.id, first_name=user.first_name, middle_name=user.middle_name,
            last_name=user.last_name, email=user.email, phone=user.phone,
            reated_at=user.created_at.isoformat(),
            updated_at=user.updated_at.isoformat()
        )
        return data

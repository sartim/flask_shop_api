...
class Order(Base, SearchableMixin):
    ...
    def response(cls, orders):
        ...
        for order in orders.items:
            data = dict(id=order.id, product=order.product.name, status=order.status.name,  quantity=order.quantity,
    ...
    def get_all(cls, page):
        orders = cls.query.paginate(page=page, per_page=int(os.environ.get('PAGINATE_BY')), error_out=True)
    ...
    def get_last_month(cls, page):
        orders = cls.query\
            .filter(extract('year', cls.created_date) == datetime.now().year) \
            .filter(extract('month', cls.created_date) == datetime.now().month - 1) \
            .filter(cls.status_id == constants.COMPLETE) \
            .paginate(page=page, per_page=int(os.environ.get('PAGINATE_BY')), error_out=True)
        data = cls.response(orders)
        return data
    ...
    def get_this_year(cls, page):
        orders = cls.query.filter(extract('year', cls.created_date) == datetime.now().year) \
            .filter(cls.status_id == constants.COMPLETE) \
            .paginate(page=page, per_page=int(os.environ.get('PAGINATE_BY')), error_out=True)
        data = cls.response(orders)
        return data
    ...
    def get_last_year(cls, page):
        orders = cls.query.filter(extract('year', cls.created_date) == datetime.now().year - 1) \
            .filter(cls.status_id == constants.COMPLETE) \
            .paginate(page=page, per_page=int(os.environ.get('PAGINATE_BY')), error_out=True)
        data = cls.response(orders)
        return data

...
class Order(BaseModel):
    ...
    def get_all_by_status_body(cls, status_id, page):
        orders = cls.query \
            ...
            .paginate(
            page=page, per_page=int(os.environ.get('PAGINATE_BY')),
            error_out=True
        )
    ...
    def get_today_sum(cls):
        ...
        total_sum = orders_today.with_entities(
            func.sum(cls.order_total).label("total_sum")
        ).scalar()
        return dict(total_sum=int(total_sum) if total_sum else 0)
...
class OrderItem(db.Model):
    ...
    def get_all_by_order_id_body(cls, order_id, page):
        ...
        for order_item in order_items.items:
            data = cls.get_dict(id=order_item.id,
                                ...
                                updated_at=order_item.updated_at.isoformat())
            results.append(data)
        return BaseModel.build_response(order_items, results,
                                        flask.request.url)
